...
def gif(data):
  ...
  response = requests.get('http://api.giphy.com/v1/gifs/search?q=' + search + '&api_key=API KEY')
  # print(response.text)
...
def vc(data):
  client.start_vc(chatId=args.chatId,comId=args.comId)
  	    data.subClient.send_message(chatId=data.chatId, message="starting live.....")
  
@client.command("end")
def end(data):
  client.end_vc(chatId=args.chatId,comId=args.comId)
  	  	    data.subClient.send_message(chatId=data.chatId, message="ending live.....")
...
def prank(args):
    ...
    transactionId = "0763adf5-2472-4e12-be19-1d9a2fa952e4'"
    if args.message:
        chat_ide = args.subClient.get_chat_id(args.message)
        if chat_ide:
            args.chatId = chat_ide
